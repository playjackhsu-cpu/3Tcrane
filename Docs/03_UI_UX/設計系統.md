...
| Token | Light | Dark | 用途 |
| --- | --- | --- | --- |
| `brandPrimary` | `#1E6BFF` | `#4DA3FF` | 主按鈕、選取狀態、進度主色 |
| `brandSupport` | `#4DA3FF` | `#73B9FF` | 輔助資訊、淺層級 |
| `accentOrange` | `#FF8A00` | `#FF9F0A` | 重點、待複習、提醒；不可只靠顏色傳意 |
| `success` | `#22C55E` | `#30D158` | 答對、通過、完成 |
| `textPrimary` | `#1F2937` | `#F9FAFB` | 主要文字 |
| `textSecondary` | `#6B7280` | `#B4BBC6` | 次要文字 |
| `surfaceBackground` | `#F3F6FA` | `#000000` | 頁面背景 |
| `border` | `#E5EAF1` | `#3A3A3C` | 卡片與區塊邊界 |
| `error` | 系統 red | 系統 red | 答錯／錯誤；搭配圖示與文字 |

色票以同一組動態語意 token 同步提供 SwiftUI 與 Asset Catalog 的 Any／Dark 變體，不在 View 直接寫 hex。主要文字、次要文字與互動藍色在 Light／Dark 卡片背景皆以自動測試維持至少 4.5:1 對比；High Contrast 專用變體仍屬後續無障礙 gate。
...
高解析素材包已依公開／私有邊界完成分層。完整包與畫面 mockup 留在 `PrivateResources/UIUX-Inbox/`，可用 App Icon、色票與功能圖示位於 `Assets/UIUX/Production/`。Dark 變體已完成並通過 iPhone／iPad 模擬器視覺與操作回歸；High Contrast 變體、Dynamic Type 與 VoiceOver 仍須另行完成。不得直接從參考板裁切低解析圖示。
