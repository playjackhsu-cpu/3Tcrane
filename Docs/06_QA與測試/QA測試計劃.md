...
4. 再答正確後依規則更新掌握狀態，但不改寫歷史作答。
5. 正式模擬固定抽 80 題（06100 取 64 題，90006～90009 各取 4 題）、限時 100 分鐘；未交卷可恢復，交卷前不揭露答案，允許空白題交卷，時間到自動交卷，交卷後分數與逐題結果固定。
6. iPhone 從「繼續學習」進題目後可直接切其他 tab。
7. iPad 從同一路徑可用 sidebar 切換，detail 不殘留題目 stack。
8. bundle B 新增／修訂／下架題目後，UserStore 紀錄完整。
9. content decode／migration／磁碟錯誤不建立空 store 覆蓋資料。
10. 飛航模式重啟，除使用者主動開啟來源網址外不需要網路。
11. 正式 bundle 不含 synthetic fixture、原始題庫、私有路徑或審查資料。
