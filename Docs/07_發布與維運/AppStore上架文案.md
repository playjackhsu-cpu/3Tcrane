...
• 錯題複習：集中重做答錯的題目；答對 1 次即移出。若在錯題複習中答錯，須再連續答對 2 次。
...
```text
新增全題庫依序練習：不限時間，題目與選項完全依內建題庫原始順序呈現，並保存下次續作位置；作答後立即顯示正確答案與解析。既有進度、錯題、收藏、筆記與測驗紀錄均保留。
```

## What's New（1.0.2）

```text
修正錯題複習作答後可能跳到另一題、造成答案與解析顯示錯配的問題。作答回饋會留在原題，只有按「下一題」或「上一題」才切換。錯題答對 1 次即可移出；若在錯題複習中答錯，須重新連續答對 2 次。既有答對 1／2 次的錯題也會依新規則移出，原學習紀錄保留。
```
